...
// .wrangler/tmp/bundle-rs57mF/strip-cf-connecting-ip-header.js
...
__name(tileIdFromLatLng, "tileIdFromLatLng");
var TREE_SPECIES = ["oak", "pine", "birch", "willow", "maple"];
var STRUCTURE_KINDS = ["house", "well", "tower", "farm"];
...
var WelcomeMessageSchema = external_exports.object({
  type: external_exports.literal("WELCOME"),
  planetId: external_exports.string(),
  serverTime: external_exports.number(),
  tickRate: external_exports.number(),
  you: external_exports.object({ sessionId: external_exports.string() })
});
var SnapshotMessageSchema = external_exports.object({
  type: external_exports.literal("SNAPSHOT"),
  planetId: external_exports.string(),
  snapshotVersion: external_exports.number(),
  serverTime: external_exports.number(),
  tiles: external_exports.record(external_exports.any()),
  entities: external_exports.record(external_exports.any())
});
var DiffMessageSchema = external_exports.object({
  type: external_exports.literal("DIFF"),
  planetId: external_exports.string(),
  seq: external_exports.number(),
  serverTime: external_exports.number(),
  tilesChanged: external_exports.array(external_exports.any()).optional().default([]),
  entitiesUpsert: external_exports.array(external_exports.any()).optional().default([]),
  entitiesRemove: external_exports.array(external_exports.string()).optional().default([])
});
var RejectedMessageSchema = external_exports.object({
  type: external_exports.literal("REJECTED"),
  planetId: external_exports.string(),
  requestId: external_exports.string(),
  reason: external_exports.string()
});
var IntentPlantTreeSchema = external_exports.object({
  type: external_exports.literal("INTENT_PLANT_TREE"),
  requestId: external_exports.string(),
  planetId: external_exports.string(),
  lat: external_exports.number().min(-90).max(90),
  lng: external_exports.number().min(-180).max(180),
  species: external_exports.enum(["oak", "pine", "birch", "willow", "maple"])
});
var IntentWaterTileSchema = external_exports.object({
  type: external_exports.literal("INTENT_WATER_TILE"),
  requestId: external_exports.string(),
  planetId: external_exports.string(),
  tileId: external_exports.string(),
  amount: external_exports.number().min(0.01).max(1)
});
var IntentBuildSchema = external_exports.object({
  type: external_exports.literal("INTENT_BUILD"),
  requestId: external_exports.string(),
  planetId: external_exports.string(),
  lat: external_exports.number().min(-90).max(90),
  lng: external_exports.number().min(-180).max(180),
  kind: external_exports.enum(["house", "well", "tower", "farm"])
});
var PingSchema = external_exports.object({
  type: external_exports.literal("PING"),
  t: external_exports.number()
});
var ClientMessageSchema = external_exports.union([
...
// src/db.ts
async function ensurePlanet(db, planetId, name, seed) {
  await db.sql`
...
__name(ensurePlanet, "ensurePlanet");
async function insertEvent(db, planetId, sessionId, actionType, payload) {
  await db.sql`
...
async function saveSnapshot(db, planetId, version, snapshot) {
  await db.sql`
...
async function getLatestSnapshot(db, planetId) {
...
__name(getLatestSnapshot, "getLatestSnapshot");

// src/PlanetDO.ts
var SMALL_TICK_MS = 2e3;
var BIG_TICK_MS = 6e4;
var ACTION_COOLDOWN_MS = 2e3;
var MOISTURE_DECAY = 2e-3;
var POLLUTION_DRIFT = 1e-3;
var FERTILITY_CHANGE_RATE = 1e-3;
var TREE_GROWTH_RATE = 0.02;
var TREE_HEALTH_DECAY = 0.01;
var THRESHOLD = 0.01;
var MAX_ENTITIES_PER_TILE = 10;
var SNAPSHOT_EVERY_EVENTS = 100;
var PlanetDO = class extends DurableObject {
  planetId;
  ...
  async handleWebSocket(request, planetId) {
    ...
    const db = this.getDb();
    if (db) {
      await ensurePlanet(db, planetId, `Planet ${planetId}`, {});
      await this.loadFromDb(db, planetId);
    } else {
    ...
    server.addEventListener("message", async (event) => {
      try {
        ...
        if (db) {
          ...
          const payload = msg.type === "INTENT_PLANT_TREE" ? { lat: msg.lat, lng: msg.lng, species: msg.species } : msg.type === "INTENT_WATER_TILE" ? { tileId: msg.tileId, amount: msg.amount } : { lat: msg.lat, lng: msg.lng, kind: msg.kind };
          await insertEvent(db, this.planetId, sessionId, actionType, payload);
          this.eventCountSinceSnapshot++;
          if (this.eventCountSinceSnapshot >= SNAPSHOT_EVERY_EVENTS) {
            this.snapshotToDb(db);
          }
        }
      } catch {
      }
  ...
  handlePlantTree(msg, sessionId) {
    if (!TREE_SPECIES.includes(msg.species)) {
  ...
  handleWaterTile(msg, _sessionId) {
  ...
  handleBuild(msg, _sessionId) {
    if (!STRUCTURE_KINDS.includes(msg.kind)) {
      return "invalid_kind";
    }
    const tileId = tileIdFromLatLng(msg.lat, msg.lng);
    const count = [...this.entities.values()].filter(
  ...
  runSmallTick() {
    ...
    for (const [tileId, tile] of this.tiles) {
      ...
      tile.fertility = Math.min(
        ...
        Math.max(
          0,
          tile.fertility + (tile.moisture > 0.3 && tile.pollution < 0.5 ? FERTILITY_CHANGE_RATE : -FERTILITY_CHANGE_RATE)
        )
      );
      if (Math.abs(tile.moisture - prev.moisture) > THRESHOLD || Math.abs(tile.fertility - prev.fertility) > THRESHOLD || Math.abs(tile.pollution - prev.pollution) > THRESHOLD) {
    ...
    for (const entity of this.entities.values()) {
      if (entity.type === "tree") {
        ...
        if (tile) {
          ...
          if (Math.abs(entity.health - prevHealth) > THRESHOLD || Math.abs(entity.growthStage - prevStage) > THRESHOLD) {
  ...
  deriveBiome(tile) {
    if (tile.moisture < 0.2 && tile.fertility < 0.3)
      return "desert";
    if (tile.moisture > 0.8)
      return "wetland";
    if (tile.moisture > 0.6 && tile.fertility > 0.5)
      return "forest";
    if (tile.moisture > 0.3 && tile.fertility > 0.4)
      return "grassland";
    if (tile.moisture < 0.4 && tile.pollution > 0.5)
      return "tundra";
    return "grassland";
  }
  buildSnapshot() {
    const tiles = {};
    const entities = {};
    for (const [k, v2] of this.tiles)
      tiles[k] = v2;
    for (const [k, v2] of this.entities)
      entities[k] = v2;
    return {
      type: "SNAPSHOT",
      planetId: this.planetId,
  ...
  buildDiffWithChanges(tilesChanged, entitiesUpsert, entitiesRemove) {
    this.seq++;
    return {
      type: "DIFF",
      planetId: this.planetId,
  ...
  async snapshotToDb(db) {
    this.snapshotVersion++;
    const snapshot = this.buildSnapshot();
    await saveSnapshot(db, this.planetId, this.snapshotVersion, snapshot);
    this.eventCountSinceSnapshot = 0;
  }
  getDb() {
    if (!this.env.DATABASE_URL)
      return null;
    return { sql: Xs(this.env.DATABASE_URL) };
  }
  async handleGetState(planetId) {
    if (this.tiles.size === 0) {
      this.ensureInitialState(planetId);
    }
    ...
    if (planetMatch) {
      ...
      if (request.method === "GET") {
        if (!subpath || subpath === "snapshot" || subpath === "snapshot/latest") {
          const stateUrl2 = `https://planet.local/state?planetId=${planetId}`;
          const res = await stub.fetch(stateUrl2, { method: "GET" });
...
// .wrangler/tmp/bundle-rs57mF/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-rs57mF/middleware-loader.entry.ts
